            ...
            match manifest.manifest_type {
                crate::manifest::ManifestType::ForgeLua => {
                    ...
                    match status {
                        ...
                        Ok(s) => ui::print_error(&format!("forge.lua build() failed with status: {}", s)),
                ...
                crate::manifest::ManifestType::AutoRust => {
                    ...
                    ui::print_info(&format!("Auto-detected Rust project (Cargo.toml). Building {}...", bin_name));
                    // Check cache
                    ...
                    match status {
                        Ok(s) if s.success() => {
                            ui::print_success(&format!("Rust build succeeded. Binary: {}", bin_path));
                            // Cache the binary
