...
pub fn run() {
    ...
    match manifest {
        Some(manifest) => {
            ...
            match manifest.manifest_type {
                ...
                crate::manifest::ManifestType::PKGBUILD => {
                    ...
                    match status {
                        Ok(s) if s.success() => {
                            ...
                            let pkg = fs::read_dir(".")
                                .ok()
                                .and_then(|entries| entries.filter_map(|e| e.ok()).find(|e| e.file_name().to_string_lossy().ends_with(".pkg.tar.zst")));
                            if let Some(pkg) = pkg {
                                let pkg_path = pkg.file_name();
                                println!("Install {} with pacman -U? [Y/n]", pkg_path.to_string_lossy());
                                ...
                                if input.trim().is_empty() || input.trim().eq_ignore_ascii_case("y") {
                                    let status = Command::new("sudo")
                                        ...
                                        .status();
                                    match status {
                                        Ok(s) if s.success() => ui::print_success("Package installed with pacman."),
                                        Ok(s) => ui::print_error(&format!("pacman -U failed with status: {}", s)),
                                        Err(e) => ui::print_error(&format!("Failed to run pacman: {}", e)),
                                    }
                                } else {
                                    println!("Install skipped.");
                                }
                            } else {
                                ui::print_error("No .pkg.tar.zst file found after packaging. Check your PKGBUILD's package() output.");
                            }
                        }
                ...
                crate::manifest::ManifestType::AutoRust => {
                    // Try to find the binary in target/release
                    let cargo_toml = fs::read_to_string("Cargo.toml").unwrap_or_default();
                    let bin_name = cargo_toml.lines().find_map(|l| {
                    ...
                    if !fs::metadata(&bin_path).is_ok() {
                        ui::print_error(&format!("Binary {} not found. Run 'forge build' first.", bin_path));
                        return;
                    }
        ...
        None => {
            ui::print_error("No PKGBUILD, forge.toml, or Cargo.toml found in the current directory or specified path. Try 'ghostforge init' to scaffold a manifest.");
        }
    }
}
